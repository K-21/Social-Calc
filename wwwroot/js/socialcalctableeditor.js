...
SocialCalc.InputBox = function(element, editor) {
   ...
   // Debounced auto-commit while typing: commit the edit after 1s of inactivity,
   // but avoid committing while in partial pointing/expression mode.
   var autoCommitTimer = null;
   var scheduleAutoCommit = function() {
      if (autoCommitTimer) window.clearTimeout(autoCommitTimer);
      autoCommitTimer = window.setTimeout(function() {
         try {
            if (editor.state === "input" && !editor.workingvalues.partialexpr) {
               editor.EditorSaveEdit();
            }
         } catch (e) { /* swallow */ }
      }, 1000);
   ...
   element.addEventListener && element.addEventListener('input', function() {
      // Only schedule auto-commit when actively editing
      if (editor.state === "input") scheduleAutoCommit();
   });

   element.addEventListener && element.addEventListener('blur', function() {
      if (autoCommitTimer) { window.clearTimeout(autoCommitTimer); autoCommitTimer = null; }
      // Ensure final commit on blur
      try { if (editor.state === "input") editor.EditorSaveEdit(); } catch (e) { }
   });
