...
AspiringStock.loadStatement = function(ticker, typ, period) {
    ...
    var xhr = $.get(url, {}, function(text) {
                    ...
                    if (tab == null) {
                    AspiringStock.tickerInfo.errorReason = "Ticker not available. Please re-check the ticker symbol, and write to us at demo@example.com";
                    AspiringStock.updateState(false);
                    return;
                    }
                    ...
                    AspiringStock.tickerInfo.errorReason = "Valuation models for financial companies are not supported in this app. To submit a request, please write to us at demo@example.com";
                    AspiringStock.updateState(false);
                    return;
                    }
                    ...
                    if (!tab) {
                    AspiringStock.tickerInfo.errorReason = "Failed to fetch the ticker data. Please try again, or write to us at demo@example.com";
                    AspiringStock.updateState(false);
                    return;
                    }
