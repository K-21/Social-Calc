...
Aspiring.loadStatement = function(ticker, typ, period) {
    ...
    var xhr = $.get(url, {}, function(text) {
	      ...
	      if (tab == null) {
		  Aspiring.tickerInfo.errorReason = "Ticker not available. Please re-check the ticker symbol, and write to us at demo@example.com.";
		  Aspiring.updateState(false);
	      	  return;
	      }
...
//		 !(Aspiring.tickerInfo.isFinancialsSupported)) {
		  ...
		  Aspiring.tickerInfo.errorReason = "Valuation models for financial companies are not supported in this app. To submit a request, please write to us at demo@example.com.";
		  Aspiring.updateState(false);
	      	  return;		  
	      }
	      ...
	      if (!tab) {
		  Aspiring.tickerInfo.errorReason = "Failed to fetch the ticker data. Please try again, or write to us at demo@example.com.";
		  Aspiring.updateState(false);
		  return;
	      }
	      // save the raw data
	      // console.log removed (tab json)
	      Aspiring.tickerInfo.rawData[typ] = tab;
	      // console.log removed (done analyze)
	      var sheet = Aspiring.convertTableToSheet(tab, 2, 
						       Aspiring.getDataSource().numtextrows);
	      // console.log removed (got typ)
	      Aspiring.tickerInfo[typ] = sheet;
	      // console.log removed (Done typ)
	      Aspiring.updateState(true);	      
	   }
	  )
    .fail(function(){
...
Aspiring.updateState = function(result, timeout) {
    ...
    // console.log removed - fsm update
    if (result) {
	Aspiring.tickerInfo.loading = false;
...
Aspiring.buildAndInsert = function() {
    // console.log removed - lengths hidden
